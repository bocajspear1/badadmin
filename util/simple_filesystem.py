...
##  Class that represents a single file and provides simplified access to using and modifing it
#
...
## Class that represents a single directory and provides simplified access to using and modifing it
#
class simple_dir(simple_fs_item):
	...
	# @returns bool - True if successful, False if not
	#
	def recursive_chmod(self, owner, group, other, special=0):
		for subdir, dirs, files in os.walk(self.__dir):

			tmp_dir = simple_dir(subdir)
			result = tmp_dir.set_permissions(owner, group, other, special)
			if result == False:
				return False
			...
			for file in files:
				...
				result = tmp_file.set_permissions(owner, group, other, special)
				if result == False:
					return False
		return True
